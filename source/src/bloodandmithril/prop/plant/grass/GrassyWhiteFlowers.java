...
/**
 * Green grass with some white flowers
 *
 * @author Matt
 */
...
public class GrassyWhiteFlowers extends PlantProp implements StaticallyRenderedProp {
	private static final long serialVersionUID = 6275732858715186393L;
	
	public static TextureRegion textureRegionBig, textureRegionSmall;

	static {
		if (ClientServerInterface.isClient()) {
			textureRegionBig = new TextureRegion(Textures.GAME_WORLD_TEXTURE, 1125, 68, 52, 50);
			textureRegionSmall = new TextureRegion(Textures.GAME_WORLD_TEXTURE, 1089, 68, 35, 50);
		}
	}

	private boolean small;

	public GrassyWhiteFlowers(final float x, final float y, final boolean small) {
		super(x, y, small ? 35 : 52, 50, Depth.FRONT, new NotEmptyTile(), false);
		this.small = small;
	}
			...
			new MenuItem(
				...
				() -> {
					Wiring.injector().getInstance(UserInterface.class).addLayeredComponent(
						new MessageWindow(
							"Some grass, there are some ordinary white flowers too.",
							Color.ORANGE,
							500,
							250,
							"Weeds",
