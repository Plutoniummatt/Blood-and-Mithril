...
public class Campfire extends CraftingStation implements Lightable {
	...
	public void update(float delta) {
		...
		if (lit && isOnScreen(position, 50f)) {
			ParticleService.randomVelocityDiminishing(position.cpy().add(0, 13f), 7f, 15f, Color.ORANGE, Util.getRandom().nextFloat() * 3f, Util.getRandom().nextFloat() * 10f, MovementMode.EMBER, Util.getRandom().nextInt(1000), Depth.MIDDLEGROUND, false, Color.RED);
			ParticleService.randomVelocityDiminishing(position.cpy().add(0, 13f), 7f, 15f, Color.ORANGE, Util.getRandom().nextFloat() * 3f, Util.getRandom().nextFloat() * 10f, MovementMode.EMBER, Util.getRandom().nextInt(1000), Depth.MIDDLEGROUND, false, Color.RED);
