...
public class FluidStripPopulator {
	...
	public Optional<FluidStrip> createFluidStripIfBase(World world, int tileX, int tileY, float volumePercent) throws NoTileFoundException {
		...
		if (!world.fluids().getFluid(tileX, tileY).isPresent() && world.getTopography().getTile(tileX, tileY, true).isPassable()) {
			try {
				// left
	 ...
	 * with a full width {@link FluidStrip} that covers the bottom most empty "layer" that is valid for a {@link FluidStrip}
	 */
	public Optional<FluidStrip> createFluidStrip(World world, int tileX, int tileY, float volumePercent) {
		int left = tileX;
		int right = tileX;
		...
		try {
			if (!world.fluids().getFluid(tileX, tileY).isPresent() && world.getTopography().getTile(tileX, tileY, true).isPassable()) {
				// left
				while (
					(world.getTopography().getTile(left, tileY, true).isPassable() && !world.fluids().getFluid(left, tileY).isPresent()) && 
					(!world.getTopography().getTile(left, tileY - 1, true).isPassable() || world.fluids().getFluid(left, tileY - 1).isPresent())
				) {
					left--;
				}
				...
				// right
				while (
					(world.getTopography().getTile(right, tileY, true).isPassable() && !world.fluids().getFluid(right, tileY).isPresent()) && 
					(!world.getTopography().getTile(right, tileY - 1, true).isPassable() || world.fluids().getFluid(right, tileY - 1).isPresent())
				) {
					right++;
				}
				...
				if ((!world.getTopography().getTile(right, tileY, true).isPassable() || world.fluids().getFluid(right, tileY).isPresent()) && 
					(!world.getTopography().getTile(left, tileY, true).isPassable() || world.fluids().getFluid(left, tileY).isPresent())) 
				{
					FluidStrip strip = new FluidStrip(left + 1, tileY, right - left - 1, (right - left - 1) * volumePercent, world.getWorldId());
					world.fluids().addFluidStrip(strip);
				...
				Optional<FluidStrip> stripBelow = world.fluids().getFluid(tileX, tileY - 1);
				...
				if (stripBelow.isPresent()) {
					...
					FluidStrip strip = new FluidStrip(restrictedLeft, tileY, restrictedRight - restrictedLeft, (restrictedRight - restrictedLeft) * volumePercent, world.getWorldId());
					world.fluids().addFluidStrip(strip);
					return Optional.of(strip);
				}
			}
