...
/**
 * A {@link CompositeAITask} that instructs the host to go to a location and plant a {@link Seed}
 *
 * @author Matt
 */
...
public class PlantSeed extends CompositeAITask {
	 ...
	 * Constructor
	 */
	public PlantSeed(Individual host, Seed toPlant) {
		super(
			host.getId(), 
			"Planting seed",
			new GoToLocation(
	...
	public class Plant extends AITask {
		...
		public void execute(float delta) {
			planted = true;
			Domain.getProps().put(toPlant.id, toPlant);
			Domain.getIndividuals().get(hostId.getId()).takeItem(toPlant.getSeed());
